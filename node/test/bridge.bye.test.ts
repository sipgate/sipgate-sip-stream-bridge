 ...
 * down. The fix echoes the INVITE's Record-Route set as Route headers (in
 * received order) and routes the BYE to RR[0] — the proxy adjacent to us, where
 * our 200 OK was accepted.
 */
...
describe('in-dialog BYE routing', () => {
  it('emits Route headers in RECEIVED Record-Route order', () => {
  ...
  it('routes the BYE to the adjacent proxy (RR[0]), not the Contact', () => {
    const rr = ['<sip:217.10.68.150;lr>', '<sip:217.10.79.9;lr>', '<sip:217.10.77.5:5060;lr>'];
    // Adjacent proxy = first Record-Route (where our 200 OK was accepted).
    expect(byeSendTarget(rr, base.remoteTarget)).toEqual(['217.10.68.150', 5060]);
  });

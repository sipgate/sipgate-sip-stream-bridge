...
|---|-------|--------|----------|
| 1 | Running the binary without required env vars prints a descriptive error naming the missing variable and exits non-zero | VERIFIED | Live test: `./audio-dock` outputs `{"level":"fatal","msg":"configuration error","error":"env: SIP_USER SIP_PASSWORD SIP_DOMAIN SIP_REGISTRAR WS_TARGET_URL SDP_CONTACT_IP are required but not set"}` and exits 1 |
| 2 | Running the binary with all required env vars starts and emits structured JSON log lines (zerolog format) to stdout | VERIFIED | Live test: binary emits `{"level":"info","sip_user":"test",...,"message":"audio-dock starting"}` and `{"level":"info",...,"message":"scaffold ready — waiting for signal"}` to stdout |
| 3 | `docker build` produces a static binary image using `FROM scratch` with `CGO_ENABLED=0`; image under 20 MB | VERIFIED | Image `audio-dock:latest` = 1.06 MB (1,115,953 bytes). Dockerfile has `FROM scratch` + `CGO_ENABLED=0`. |
...
| 6 | `go build ./cmd/audio-dock` succeeds with zero warnings | VERIFIED | `go build ./cmd/audio-dock && echo "BUILD OK"` produces `BUILD OK` with no output |
...
|----------|----------|--------|---------|
| `go.mod` | Go module definition `github.com/sipgate/audio-dock`, go 1.25 | VERIFIED | `module github.com/sipgate/audio-dock`, `go 1.25.0` (canonical toolchain form); zerolog v1.34.0 and go-simpler.org/env v0.12.0 in require block |
...
| `cmd/audio-dock/main.go` | Entry point: config.Load(), zerolog logger, startup log, signal wait | VERIFIED | 51 lines (>30 min); calls config.Load(), uses zerolog.New(os.Stdout).With().Timestamp().Logger(), logs all config fields, waits on signal.NotifyContext |
| `Dockerfile` | Two-stage: golang:1.26-alpine builder + FROM scratch runtime with CA certs | VERIFIED | FROM golang:1.26-alpine AS builder + FROM scratch; COPY CA certs; go build ./cmd/audio-dock with CGO_ENABLED=0 |
| `docker-compose.yml` | Compose file with network_mode: host and env_file: .env | VERIFIED | `network_mode: host`, `env_file: - .env`, `build: .`, `image: audio-dock:latest` |
...
|------|----|-----|--------|---------|
| `cmd/audio-dock/main.go` | `internal/config/config.go` | `config.Load()` call | WIRED | Line 16: `cfg := config.Load()` |
| `cmd/audio-dock/main.go` | `github.com/rs/zerolog` | `zerolog.New(os.Stdout).With().Timestamp().Logger()` | WIRED | Line 21: `logger := zerolog.New(os.Stdout).With().Timestamp().Logger()` |
| `internal/config/config.go` | `go-simpler.org/env` | `env.Load(&cfg, nil)` | WIRED | Line 41: `if err := env.Load(&cfg, nil); err != nil {` |
| `Dockerfile` | `cmd/audio-dock` | `go build ./cmd/audio-dock` in RUN step | WIRED | Lines 17–21: multi-line RUN with `go build … ./cmd/audio-dock` |
...
Phase 4 goal is fully achieved. The codebase contains a runnable Go binary (`cmd/audio-dock/main.go`) that:
